    ...
    T: UnsignedInteger,
{
    ...
    #[inline]
    pub fn add<M, A>(mut self, rhs: &CrtPolynomial<A, T>, moduli: &[M], poly_length: usize) -> Self
    where
    ...
    T: UnsignedInteger,
{
    /// Performs `self += rhs` according to `moduli`.
    #[inline]
    pub fn add_assign<M, A>(&mut self, rhs: &CrtPolynomial<A, T>, moduli: &[M], poly_length: usize)
    where
        M: Copy + ReduceAddAssign<T>,
        A: RawData<Elem = T> + Data,
    {
